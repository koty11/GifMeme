...
gifmeme.generate = function(file, topText, bottomText, next){
    ...
    gm(file).size(function(err, size){
        ...
        gm(file).font("./impact.ttf", 30).stroke("#000000").fill('#ffffff').gravity('North').drawText(0, topFontSize+15 , topText).drawText(0, height-15 , bottomText).write(outputDirectory + memefilename, function (err) {
            if (err) {
                return next(err);
            } else {
